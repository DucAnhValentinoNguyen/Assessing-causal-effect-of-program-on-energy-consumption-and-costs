...
outcomes_wide <- df %>%
  ...
  mutate(
    ...
    # first Difference in Energy Consumption 
    dY_cost_2020 = dml_data$energy_cost_2020 - dml_data$energy_cost_2018,
    dY_cost_2023 = dml_data$energy_cost_2023 - dml_data$energy_cost_2018,
...
head(dml_data)
save(dml_data, file = "./data/clean_dml_data.RData")
```
...
print(res_cost_B)
```
Quick interpretation: Households in Program A regions reduced their monthly energy consumption by approximately 100 kWh (significant result) while Program B had no statistically significant impact on energy use.
...
print(res_health_B)
```
Quick interpretation: Program A (Incentives): The estimate is -0.1672 with a p-value $< 0.01$: participation in Program A led to an average reduction of 0.17 health issues per year per household. We cannot say with certainty that Program B had any effect on health.
...
```
COMPARE: this estimate is similar to our Lasso estimate (-112.5 vs -99.8), we can argue our result is robust to functional form assumptions.
...
```
Quick interpretation: undertaking a retrofit reduces monthly energy consumption
by 210.20 kWh (hat(β_IV) = −210.20, SE = 15.17) and monthly energy costs by 103.50
EUR
